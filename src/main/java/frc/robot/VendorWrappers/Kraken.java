...
/**
 * A wrapper class around TalonFX that provides some convience features, like auto-retyring configs
 * if they fail.
 */
public class Kraken extends TalonFX {
  ...
  public void applyConfig(TalonFXConfiguration config) {
    ...
    while (statorCurrentLimit == 0) {
      ...
      System.out.println(
          "u gotta set something for TalonFXConfiguration.CurrentLimits.StatorCurrentLimit");
      Timer.delay(5);
    }
    while (!currentLimitEnabled) {
      System.out.println("bruh, u forgot to enable the current limit for " + name);
      System.out.println(
          "u gotta explicitly set TalonFXConfiguration.CurrentLimits.StatorCurrentLimitEnable to true");
      Timer.delay(5);
    }
    ...
    if (statorCurrentLimit >= 100) {
      ...
      System.out.println(
          "Make sure to lower it after you're done testing whatever it is you're testing!");
    }
    ...
    while (true) {
      ...
      /**
       * Verify all configs match their expected values. Note that we have to compare the configs
       * using their String representation, because CTRE did not override the .equals() method for
