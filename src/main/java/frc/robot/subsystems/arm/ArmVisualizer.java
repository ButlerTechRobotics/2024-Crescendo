...
public class ArmVisualizer {
  ...
  public void update(double angle) {
    armLigament.setAngle(Units.radiansToDegrees(angle));

    Logger.recordOutput("Mechanism2d/" + logKey, mechanism);
    // Transform3d hardpoint =
    // new Transform3d(
    // Units.inchesToMeters(12.25), 0, Units.inchesToMeters(25.15), new
    // Rotation3d(0, 0,
    // 0));

    // Pose3d armPose = new Pose3d(Units.inchesToMeters(10), 0, 0, new Rotation3d(0,
    // armAngle, 0));
    Pose3d armPose = getArmPose(angle);
  ...
  private Pose3d getArmPose(double angle) {
    return new Pose3d(armRoot.getX(), 0, armRoot.getY(), new Rotation3d(0, -angle, 0));
  }
}
