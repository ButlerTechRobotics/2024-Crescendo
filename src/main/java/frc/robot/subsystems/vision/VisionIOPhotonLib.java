...
public class VisionIOPhotonLib implements VisionIO {
    ...
    public VisionIOPhotonLib(String identifier, Transform3d robotToCamera) {
    ...
    public void updateInputs(VisionIOInputs inputs) {
        ...
        inputs.nearestTagDistanceMeters = result.getBestTarget().getBestCameraToTarget().getTranslation()
        ...
        // if there's only one tag on the screen, only trust it if its pose ambiguity is
        // below threshold
        // photonPoseEstimator automatically switches techniques when detecting
        // different number of tags
        if (result.targets.size() > 1
                || (result.targets.size() == 1 && result.getBestTarget().getPoseAmbiguity() < 0.2)) {
