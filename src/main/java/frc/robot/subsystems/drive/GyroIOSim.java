...
public class GyroIOSim implements GyroIO {
  ...
  /**
   * Because the gyroscope does not exist in simulation, this implementation serves to simulate the
   * gyro.
   *
   * <p>Using odometry, the yaw can be calculated. In order to do this, call {@code
   * calculateYawFromOdometry()} periodically.
   */
  ...
  public GyroIOSim() {
    ...
    for (int i = 0; i < 4; i++) {
      prevModulePositions[i] = new SwerveModulePosition(0, new Rotation2d(0));
    }
  }

  /**
   * Updates yaw of simulated gyro using odometry. Call this method periodically in order for it to
   * work.
   *
   * @param modulePositions - an array of current module positions, in the same order as your
    ...
    for (int i = 0; i < modulePositions.length; i++) {
      moduleDeltas[i] =
          new SwerveModulePosition(
              ...
              modulePositions[i]
                  .angle); // the angle here should NOT be the change in angle, draw it out if you
      // can't see why
    }
