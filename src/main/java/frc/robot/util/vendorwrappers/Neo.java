...
public class Neo extends CANSparkMax {
  ...
  // physics constants
  /**
   * How many volts were applied to the motor by the manufacturer when determining its electrical
   * characteristics.
   */
  public static final double nominalVoltage = 12;

  /**
   * How quickly the motor turns (radians per second) when 12 volts are applied and there is no load
   * on the motor.
   */
  ...
  /**
   * How much current (amps) flows through the motor coils when the rotor is held still, and then 12
   * volts are applied to the motor.
   */
  public static final double stallCurrent = 181;

  /**
   * How much torque (newton-meters) the stator will exert on the rotor per amp of current that
   * flows through the motor coils.
   */
  ...
  /**
   * How many volts are induced in the motor windings by the permanent magnets on the rotor for each
   * radian-per-second that the rotor is spinning at.
   */
  public static final double kEMF =
      (nominalVoltage - (freeCurrent * windingResistance)) / freeSpeed;
  ...
  public void setInverted(boolean isInverted) {
    ...
    this.waitForConfig(
        () -> {
          ...
          return super.getLastError(); /* idk if this works */
        },
        errorMessage);
  }
  ...
  public REVLibError setSmartCurrentLimit(int limitAmps) {
    String errorMessage =
        "Failed to set a current limit of " + limitAmps + " amps for " + name + "!";
    return this.waitForConfig(
        () -> {
  ...
  public REVLibError setPositionConversionFactor(double factor) {
    String errorMessage =
        "Failed to set " + name + "'s positionConversionFactor to " + factor + "!";
    return this.waitForConfig(
        () -> {
  ...
  public REVLibError setVelocityConversionFactor(double factor) {
    String errorMessage =
        "Failed to set " + name + "'s velocityConversionFactor to " + factor + "!";
    return this.waitForConfig(
        () -> {
  ...
  /**
   * Finds the amount of volts needed to feed to the motor in order for the motor to exert a certain
   * torque.
   */
